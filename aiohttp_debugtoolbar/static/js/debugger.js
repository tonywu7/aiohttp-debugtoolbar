...
pyramid_debugtoolbar_require(['jquery', 'toolbar'], function ($, tablesorter, toolbar) {
    $(function () {
        var sourceView = null
        ...
        $('div.traceback div.frame').each(function () {
            var target = $('pre', this).click(function () {
                    sourceButton.click()
                }),
                consoleNode = null,
                source = null,
                ...
                .click(function () {
                    if (!sourceView)
                        $(
                            'h2',
                            (sourceView = $(
                                '<div class="box"><h2>View Source</h2><div class="sourceview">' + '<pre></pre></div>'
                    ...
                    $.get(
                        window.DEBUG_TOOLBAR_SOURCE_PATH,
                        { frm: frameID, token: window.DEBUGGER_TOKEN },
                        function (data) {
                            var dataLine
                            ...
                            var code =
                                '<pre data-line="' +
                                dataLine +
                                '"><code id="pDebugTracebackSrc"' +
                                'class="language-python">' +
                                data.source +
                                '</code></pre>'

                            $('pre', sourceView).replaceWith(code)
        ...
        function openShell(consoleNode, target, frameID) {
            ...
            var form = $('<form>&gt;&gt;&gt; </form>')
                .submit(function () {
                    var cmd = command.val()
                    $.get(
                        window.DEBUG_TOOLBAR_EXEC_PATH,
                        {
                            cmd: cmd,
                            frm: frameID,
                            token: window.DEBUGGER_TOKEN,
                        },
                        function (data) {
        ...
        function focusSourceBlock() {
            var tmp,
                line = $('pre code .line-highlight')
            for (var i = 0; i < 7; i++) {
                tmp = line.prev()
                if (!(tmp && tmp.is('.in-frame'))) break
                line = tmp
            }
